...
function llexamples() {
    if(rlacetheme.startsWith('ace') && runlatex.completionsURI != ""){
	...
	if(!p[i].classList.contains('CodeMirror-line-like') && !p[i].classList.contains('CodeMirror-line') ){
	...
	if(!pretext.match(noeditregex) && !p[i].classList.contains('noedit')) {
		...
		var pcont = p[i].textContent;
		p[i].textContent="";
		editor =CodeMirror(p[i],{
		    lineNumbers: true,
		    mode: 'stex',
		    tabSize: 2,
		    value: pcont
...
if(rlacetheme=="") rlacetheme="codemirror/theme/monokai";
